...
import { ChevronLeft, Truck, Package, CheckCircle, Clock, XCircle, Download } from 'lucide-react';
...
export default function DeliveryDayView() {
  ...
  const handleStatusToggle = async (orderId: string, type: 'delivery' | 'collection') => {
    try {
      // Get current status
      const currentStatus = type === 'delivery' 
        ? (deliveryStatuses[orderId] || 'pending')
        : (collectionStatuses[orderId] || 'pending');

      // Cycle through states: pending -> completed -> cancelled -> pending
      ...
      setToastMessage(`${type === 'delivery' ? 'Delivery' : 'Collection'} marked as ${newStatus}`);
  ...
  const getStatusStyles = (status: 'pending' | 'completed' | 'cancelled', type: 'delivery' | 'collection') => {
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className={`flex-1 cursor-pointer ${status === 'cancelled' ? 'line-through' : ''}`}>
