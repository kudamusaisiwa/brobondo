...
import { useOwnerDocumentStore } from '../../store/ownerDocumentStore';
...
export default function OwnerDocumentsCard({ ownerId }: OwnerDocumentsCardProps) {
  const { documents, loading, error, initialize, uploadDocument, deleteDocument } = useOwnerDocumentStore();
  ...
  useEffect(() => {
    ...
    if (ownerId) {
      console.log('Initializing documents for owner:', ownerId);
      const init = async () => {
        try {
          unsubscribe = await initialize(ownerId);
        } catch (error) {
