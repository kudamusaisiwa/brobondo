...
class ManyContactApi {
  ...
  /**
   * Sync leads from ManyContact
   * @returns {Promise<{ success: boolean, message: string, leads: Contact[] }>}
   */
  async syncLeads() {
    try {
      console.log('syncLeads called');

      // Fetch all contacts as a fallback for syncing leads
      const response = await fetch(`${this.baseURL}/contacts`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to sync leads: ${response.status} ${errorData?.message || response.statusText}`);
      }
      ...
      console.log('Processing contacts into leads...');
      const leads = contacts.map(contact => ({
        id: contact.id,
        name: [contact.firstName, contact.lastName].filter(Boolean).join(' ') || contact.phone,
        ...
        status: 'new',
        changedAt: new Date(),
        manyContactId: contact.id
      }));
      console.log('Leads processed successfully:', leads);
      return leads;
    } catch (error: any) {
      console.error('Error syncing leads:', {
        error: error.message,
        stack: error.stack
      });
      return {
